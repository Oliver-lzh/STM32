...
typedef struct
{
    uint8_t m_registrationCount;  //number of currently occupied registration slots
    ICommands *m_registrations[MAX_COMMAND_REGISTRATIONS];
} CommandHandler;
 ...
 *
 *  @param wValue contains bType Type of component to be addressed
 *  @param wIndex contains bId ID of the component in case there are multiple instances, otherwise 0
