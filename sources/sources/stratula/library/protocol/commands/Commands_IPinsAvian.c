...
static uint8_t Commands_IPinsAvian_setResetPin(IPinsAvian *pinsAvian, uint16_t wLength, const uint8_t *payload)
{
...
static uint8_t Commands_IPinsAvian_getIrqPin(IPinsAvian *pinsAvian, uint16_t wLength, uint8_t **payload)
{
    if (wLength != sizeof(uint8_t))
    {
        return STATUS_COMMAND_WLENGTH_INVALID;
...
static uint8_t Commands_IPinsAvian_reset(IPinsAvian *pinsAvian, uint16_t wLength, const uint8_t *payload)
{
    if (wLength != 0)
    {
        return STATUS_COMMAND_WLENGTH_INVALID;
