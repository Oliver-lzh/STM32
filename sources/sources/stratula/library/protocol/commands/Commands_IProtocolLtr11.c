...
static uint8_t Commands_IProtocolLtr11_executeWrite(IProtocolLtr11 *protocol, uint16_t wLength, const uint8_t *payload)
{
    ...
    else if (!(wLength % sizeof(uint16_t)))
    {
        ...
        command                   = (const uint8_t(*)[2])(uintptr_t)(payload + length);
        const uint8_t(*values)[2] = (const uint8_t(*)[2])(uintptr_t)(payload);
...
static uint8_t Commands_IProtocolLtr11_executeRead(IProtocolLtr11 *protocol, uint16_t wLengthIn, const uint8_t *payloadIn, uint16_t *wLengthOut, uint8_t **payloadOut)
{
    if (wLengthIn == sizeof(uint8_t))
    {
        uint16_t *value = (uint16_t *)(uintptr_t)(*payloadOut);
        if (*wLengthOut < sizeof(*value))
        {
    ...
    {
        const uint16_t count       = serialToHost16(payloadIn);
        const uint8_t(*command)[2] = (const uint8_t(*)[2])(uintptr_t)(payloadIn + sizeof(count));
        if (*wLengthOut < count * sizeof(uint16_t))
        {
            return STATUS_PAYLOAD_TOO_LONG;
        }
...
static uint8_t Commands_IProtocolLtr11_setBits(IProtocolLtr11 *protocol, uint16_t wLength, const uint8_t *payload)
{
    uint8_t address;
    uint16_t bitMask;
    ...
    address = serialToHost8(payload);
    bitMask = serialToHost16(payload + sizeof(address));

    return protocol->setBits(protocol, address, bitMask);
}

static uint8_t Commands_IProtocolLtr11_setting(IProtocolLtr11 *protocol, uint16_t wLength, const uint8_t *payload)
{
    uint16_t prt;
    if (wLength != sizeof(prt))
    {
        return STATUS_COMMAND_WLENGTH_INVALID;
    }

    prt = serialToHost16(payload);

    return protocol->setMisoArbitration(protocol, prt);
}

static uint8_t Commands_IProtocolLtr11_executeWriteBatch(IProtocolLtr11 *protocol, uint16_t wLength, const uint8_t *payload)
{
