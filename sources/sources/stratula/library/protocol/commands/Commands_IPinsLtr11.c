...
static uint8_t Commands_IPinsLtr11_setResetPin(IPinsLtr11 *pinsLtr11, uint16_t wLength, const uint8_t *payload)
{
...
static uint8_t Commands_IPinsLtr11_getDetectionPins(IPinsLtr11 *pinsLtr11, uint16_t wLength, uint8_t **payload)
{
    if (wLength != sizeof(uint8_t))
    {
        return STATUS_COMMAND_WLENGTH_INVALID;
...
static uint8_t Commands_IPinsLtr11_reset(IPinsLtr11 *pinsLtr11, uint16_t wLength, const uint8_t *payload)
{
    if (wLength != 0)
    {
        return STATUS_COMMAND_WLENGTH_INVALID;
