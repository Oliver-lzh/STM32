...
static void DataAvian_newDataCallback(void *arg)
{
    ...
    if (self->wordSize == 0)
    {
        ...
        const uint64_t timestamp = chrono_ticks_to_microseconds(chrono_now());
        if (!self->readouts[0][1]--)
        {
            PlatformInterrupt_enable(self->irq, false);  // disable IRQ after one frame
        }
        self->prescaler = self->readouts[0][0];

        DataQueue_t *queue            = &self->queue;
        const uint32_t writeAvailable = queue_writeAvailable(queue);
        const uint16_t sliceSize      = sizeof(timestamp);
        if (writeAvailable < sliceSize)
        {
        ...
        memcpy(data, &timestamp, sliceSize);
        queue_updateWritePointer(queue, sliceSize);
        return;
    }
...
sr_t DataAvian_configure(uint8_t index, const IDataProperties_t *dataProperties, const uint8_t *settings, uint16_t settingsSize)
{
    ...
    {
        // for timestamps as DataFormat_U64, readoutAddress is the prescaler of the interrupt counter
        if (dataProperties->format != DataFormat_U64)
        {
    ...
    switch (dataProperties->format)
    {
        ...
        case DataFormat_Packed12:
            ...
            break;
        case DataFormat_Raw16:  // one 12-bit sample is unpacked into one 16-bit word
            if (self->useQspi)
            {
            ...
            break;
        case DataFormat_U64:  // helper functionality to measure interrupt time stamps
            if (self->aggregation != 1)
            {
                return E_INVALID_PARAMETER;
            }
            self->wordSize  = 0;                                // no device data is read, we simply copy the timestamp
            self->sliceSize = readoutCount * sizeof(uint64_t);  // set number of words per frame or frame slice in bytes
            break;
        default:
            return E_INVALID_PARAMETER;
    }
