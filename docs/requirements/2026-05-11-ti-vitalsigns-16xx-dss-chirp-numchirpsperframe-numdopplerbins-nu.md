# 技术澄清：TI vitalSigns_16xx_dss 一帧由多少 chirp 组成；numChirpsPerFrame/numDopplerBins/numT...
...
## Evidence Inputs
- Source task: 技术澄清：TI vitalSigns_16xx_dss 一帧由多少 chirp 组成；numChirpsPerFrame/numDopplerBins/numTxAntennas 如何决定 radarCube；生命体征算法取哪个最小处理单位；只读分析不修改代码。
- Intent contract: intent-contract.json
- Runtime input packet: runtime-input-packet.json

## Runtime Input Truth
- Governance scope: root
- Root run id: 20260511T081019Z-e7ddc62f
- Entry intent: vibe
...
- Capsule [6df194ecac6a0a89] Task focus: 技术澄清：TI vitalSigns_16xx_dss 一帧由多少 chirp 组成；numChirpsPerFrame/numDopplerBins/numTxAntennas 如何决定 radarCube；生命体征算法取哪个最小处理单位；只读分析不修改...
  Owner: state_store
  Why now: Matched state_store memory for requirement_doc.
  Expansion Ref: C:\Users\Oliver\Desktop\in\firmware-rbb7\firmware-rbb7\Firmware\outputs\runtime\vibe-sessions\20260511T081019Z-e7ddc62f\memory-activation\skeleton-local-digest.json#6df194ecac6a0a89
  Summary: Task focus: 技术澄清：TI vitalSigns_16xx_dss 一帧由多少 chirp 组成；numChirpsPerFrame/numDopplerBins/numTxAntennas 如何决定 radarCube；生命体征算法取哪个最小处理单位；只读分析不修改代码。
  Summary: Git branch: 
